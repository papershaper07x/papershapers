...
def build_generator_prompt_questions_only(
    ...
    has_internal_choice = any(sec.get("internal_choices", 0) > 0 for sec in plan.get("sections", []))

    s = []
    s.append("You are an expert exam paper generator. IMPORTANT: RETURN ONLY A SINGLE VALID JSON OBJECT — DO NOT INCLUDE ANSWERS, RATIONALE, OR ANY TEXT OUTSIDE THE JSON.")
    s.append("Follow the planner summary and the specific question formats and counts for each section.")
    ...
    s.append("\nINSTRUCTIONS:")
    s.append("1) Produce the required number and types of questions exactly as specified in the planner.")
    s.append("2) CRITICAL: For questions with an internal choice (`is_choice: true`), the `q_text` key MUST be an array of objects. DO NOT add a separate string-based `q_text` key in the same question object, as this creates invalid JSON.")
    s.append("3) Ensure all backslashes inside JSON strings are properly escaped (e.g., use `\\\\` for a literal backslash in LaTeX).")
    ...
    s.append('''{"paper_id":"<string>","board":"CBSE","class":"<string>","subject":"<string>","questions":[<list of question items following the example formats>]}''')
...
def _find_json_substring(text: str) -> str:
    ...
    if end_pos < start_pos:
        raise ValueError("Mismatched brackets: last closing bracket/brace appears before the first opening one.")
...
def parse_generator_response(llm_text: str) -> Any:
    ...
    try:
        ...
        raise ValueError(f"Failed to locate any potential JSON substring. Original error: {e}")
