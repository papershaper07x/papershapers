...
@router.post(
    "/process", response_model=models.ProcessResponse, tags=["Legacy Generation"]
)
...
@router.post(
    "/generate", response_model=models.ProcessResponse, tags=["Legacy Generation"]
)
...
async def research_endpoint(request: Request):
    ...
    try:
        ...
        raise HTTPException(
            status_code=status.HTTP_422_UNPROCESSABLE_ENTITY, detail=ve.json()
        )
    except Exception as e:
