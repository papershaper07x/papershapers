...
if not GEMINI_API_KEY:
    LOG.warning("GOOGLE_API_KEY not found in environment. Generative calls will fail until you set it.")
    ...
    for question in questions:
        ...
        if question.get("is_choice") and isinstance(question.get("q_text"), list) and len(question["q_text"]) > 1:
            ...
            if len(set(choice_texts)) == 1:
                LOG.warning(f"Found and fixed duplicate internal choice for q_id: {question.get('q_id')}")
                question["q_text"] = choice_texts[0]  # Collapse to a single question text
                del question["is_choice"]  # It's no longer a choice
                question["_cleaning_note"] = "Collapsed duplicate internal choices into a single question."
        ...
        if "case" in question.get("type", "").lower() and isinstance(question.get("q_text"), list):
            ...
            if match:
                ...
                if passage and cleaned_sub_questions:
                    LOG.warning(f"Found and restructured case study for q_id: {question.get('q_id')}")
                    # Rebuild the q_text object in the correct format
                    question["q_text"] = {
                        "passage": passage,
                        "questions": cleaned_sub_questions
                    }
                    question["_cleaning_note"] = "Restructured case study from repeated text to a proper passage/questions format."
...
def process_section_sync(sec: Dict[str, Any], file_data: str, class_label: str, subject: str) -> Dict[str, Any]:
    ...
    try:
        ...
        base_objective = build_retrieval_objective(sec, subject_guidelines=file_data, user_mode='balanced')
        specific_objective = f"For {class_label} {subject}, find content for: {base_objective}"
        ...
        subject_value_for_pinecone = standardize_subject_name(subject) # <--- USE THE NEW FUNCTION
        filters = {'class': {'$eq': class_value_for_pinecone}, 'subject': {'$eq': subject_value_for_pinecone}}
        ...
        if not candidates:
            ...
            return {"section_id": sec.get("section_id"), "summaries": [{"id": "N/A", "summary": "No relevant evidence was found for this section."}], "slot_meta": sec.get("title", "")}
        ...
        if not valid_candidates:
             print(f"WARNING: Candidates found for Section {sec.get('section_id')} but none had embeddings.")
             return {"section_id": sec.get("section_id"), "summaries": [{"id": "N/A", "summary": "No valid evidence could be processed."}], "slot_meta": sec.get("title", "")}
        ...
        print(f"INFO: Summarizing {len(selected_snips)} snippets for Section {sec.get('section_id')}...")
        dense_summary = generate_dense_evidence_summary_with_llm(selected_snips, specific_objective, call_gemini)
        ...
        print(f"FATAL ERROR in process_section_sync for section {sec.get('section_id')}: {e}")
        ...
        return {
            "section_id": sec.get("section_id"),
            "summaries": [{"id": "ERROR", "summary": "An error occurred during evidence processing."}],
            "slot_meta": sec.get("title", "")
        }
...
async def generate(req: GenerateRequest):
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Failed to load schema row: {str(e)}")

    if not row:
        raise HTTPException(status_code=404, detail=f"Schema not found for {req.board}, {req.class_label}, {req.subject}")

    file_data = row.get("File_Data", "") or ""
    plan = derive_plan_from_filedata(file_data, req.subject)
    LOG.info(f"Derived plan with {len(plan.get('sections', []))} sections for {req.subject}.")
    ...
    planner_text = plan.get("planner_text", "Generate a standard exam paper based on the evidence.")
    prompt = build_generator_prompt_questions_only(planner_text, slot_summaries_list, plan, gen_settings={"mode": "production"})
    ...
    try:
        ...
        raise HTTPException(status_code=500, detail=f"Parse failed: {e}. Raw LLM output: {gen_resp.get('text', '')}")
...
"""
    response_text = generate_response(model="models/gemini-2.5-flash-lite", msg=answer_paper)
...
def process_subqueries_parallel(subquery_url_map):
    ...
    results = []
    with concurrent.futures.ThreadPoolExecutor() as tpe:
        future_to_task = {tpe.submit(process_url, url, subquery): (url, subquery) for url, subquery in tasks}
